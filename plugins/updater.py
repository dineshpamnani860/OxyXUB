...
from . import HELP, get_string, OxyX-UB_version

UPSTREAM_REPO_URL = "https://github.com/OxyNotOp/OxyX-UB"
...
async def gen_chlog(repo, diff):
    ...
    ch_log = tldr_log = ""
    ch = f"<b>OxyX-UB {OxyX-UB_version} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
    ch_tl = f"OxyX-UB {OxyX-UB_version} updates for {ac_br}:"
    d_form = "%d/%m/%y || %H:%M"
...
async def upstream(ups):
    ...
    if "now" not in conf:
        if changelog:
            ...
            if len(changelog_str) > 4096:
                ...
                file = open(f"OxyX-UB_updates.txt", "w+")
                file.write(tldr_str)
                file.close()
                await ups.client.send_file(
                    ups.chat_id,
                    f"OxyX-UB_updates.txt",
                    caption=get_string("upd_5").format(hndlr),
                    reply_to=ups.id,
                )
                remove(f"OxyX-UB_updates.txt")
                return
            else:
    ...
    if Var.HEROKU_API is not None:
        ...
        execl(sys.executable, sys.executable, "-m", "pyOxyX-UB")
        return
